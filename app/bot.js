...
client.on("userUpdate", async (oldUser, newUser) => {
  if (oldUser.username !== newUser.username) {
      const tag = 'た'//Tagınızı Koyun
      const sunucu = '875845839975768064'//Sunucu İD
      const kanal = '925071036846862356'//Log Kanal İD
      const rol = '925071899690672128'//Rol İd
...
.setImage("https://cdn.discordapp.com/attachments/756969726034313406/762304211446005770/giphy.gif")  
    client.channels.cache.get("875845840193867819").send(oskobs)//kanalid
})
...
client.on("guildMemberAdd", async member => {
    ...
    if (ktarih < 2592000001) {
        member.roles.add("875845840013525011")//Şüpheli Hesap
    
    }else{
    
        member.roles.add("875845840013525008")//Güvenilir Hesağ
